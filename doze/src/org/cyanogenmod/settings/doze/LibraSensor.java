...
public abstract class LibraSensor {

    private static final String TAG = "LibraSensor";

    private static final String DOZE_INTENT = "com.android.systemui.doze.pulse";

    private static final int SENSOR_WAKELOCK_DURATION = 60000; // 60sec
    ...
    public LibraSensor(Context context, int type) {
        mContext = context;
        mType = type;
    ...
    public void enable() {
		if (!mSensorWakeLock.isHeld()) {
			mSensorWakeLock.acquire(SENSOR_WAKELOCK_DURATION);
			Log.d(TAG, "disable() wakelock acquired for " + SENSOR_WAKELOCK_DURATION + " milliseconds");
		}
    ...
    public void disable() {
		...
		if (mSensorWakeLock.isHeld()) {
			mSensorWakeLock.release();
			Log.d(TAG, "disable() wakelock released");
		}
    }
